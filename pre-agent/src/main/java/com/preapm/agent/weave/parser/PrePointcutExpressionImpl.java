...
public class PrePointcutExpressionImpl {
    ...
    public PrePointcutExpressionImpl(Pointcut pointcut, String expression, PointcutParameter[] params, World inWorld) {
    ...
    public boolean couldMatchJoinPointsInType(String className) {
        ResolvedType matchType = world.resolve(className);
        if (matchType.isMissing() && (world instanceof PreReflectionWorld)) {
            matchType = ((PreReflectionWorld)world).resolveUsingClass(className);
        }
        ReflectionFastMatchInfo info = new ReflectionFastMatchInfo(matchType, null, this.matchContext, world);
        boolean couldMatch = pointcut.fastMatch(info).maybeTrue();
        if (MATCH_INFO) {
            System.out.println("MATCHINFO: fast match for '" + this.expression + "' against '" + className + "': "
                    + couldMatch);
        }
        return couldMatch;
    }
    ...
    public boolean mayNeedDynamicTest() {
        PrePointcutExpressionImpl.HasPossibleDynamicContentVisitor visitor = new PrePointcutExpressionImpl.HasPossibleDynamicContentVisitor();
